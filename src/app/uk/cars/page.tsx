...
const sortLabels: Record<SortOption, string> = {
  ...
  'price-low': 'Ціна (від низької до високої)',
  'price-high': 'Ціна (від високої до низької)',
  'deposit-low': 'Залог (від низького до високого)',
};
      ...
      <Section background="primary" padding="lg" className="border-b border-[var(--border)]">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-bold text-[var(--text)] mb-4">
            Оренда автомобілів на Тенерифі
          </h1>
          <p className="text-xl text-[var(--text-muted)] mb-8">
            Виберіть автомобіль та надішліть запит на бронювання за кілька хвилин.
          </p>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
                  ...
                  className={`
                    ...
                    ${
                      selectedCategory === category
                        ...
                        : 'bg-[var(--surface)] text-[var(--text)] border border-[var(--border)] hover:border-[#2B2B2B]'
                    }
                  `}
                >
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="px-4 py-2 rounded-lg border border-[var(--border)] bg-[var(--surface)] text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-[#2B2B2B]/20 transition-all duration-200"
              >
                ...
                <option value="deposit-low">{sortLabels['deposit-low']}</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto">
          {filteredAndSortedCars.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-xl text-[var(--text-muted)]">Не знайдено автомобілів, що відповідають вашим критеріям.</p>
            </div>
          ) : (
                    ...
                    <div className="flex-1 flex flex-col">
                      ...
                      <div className="border-t border-[var(--border)] pt-4 mb-4 space-y-2">
                        ...
                        <div className="text-sm text-[var(--text-muted)]">
                          Залог: {formatEUR(car.deposit)}
                        </div>
                      </div>

                      {/* Actions */}
                      <div className="flex flex-col sm:flex-row gap-2">
                        <Link href={`/uk/cars/${car.slug}`} className="flex-1">
                          <Button variant="outline" size="md" className="w-full">
                            Детальніше
                          </Button>
                        </Link>
                        <Link href={`/uk/booking?car=${car.slug}`} className="flex-1">
