...
const sortLabels: Record<SortOption, string> = {
  ...
  'price-high': 'Цена (от высокой к низкой)',
  'deposit-low': 'Залог (от низкого к высокому)',
};
      ...
      <Section background="primary" padding="lg" className="border-b border-[var(--border)]">
        <div className="max-w-7xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-bold text-[var(--text)] mb-4">
            Аренда автомобилей на Тенерифе
          </h1>
          <p className="text-xl text-[var(--text-muted)] mb-8">
            Выберите автомобиль и отправьте запрос на бронирование за несколько минут.
          </p>
        ...
        <div className="max-w-7xl mx-auto">
          {filteredAndSortedCars.length === 0 ? (
            <div className="text-center py-16">
              <p className="text-xl text-[var(--text-muted)]">Не найдено автомобилей, соответствующих вашим критериям.</p>
            </div>
          ) : (
                  ...
                  <ScrollReveal key={car.id} delay={index * 50}>
                    ...
                    <div className="flex-1 flex flex-col">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        <Link href={`/ru/cars/${car.slug}`} className="flex-1">
                          <Button variant="outline" size="md" className="w-full">
                            Подробнее
                          </Button>
                        </Link>
                        <Link href={`/ru/booking?car=${car.slug}`} className="flex-1">
